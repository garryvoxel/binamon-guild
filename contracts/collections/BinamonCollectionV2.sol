...
interface LegacyCollection is IERC721 {
    ...
    function tokenDetails(uint256 tokenId)
        external
        view
        returns (BinamonToken memory token, BinamonType memory type_);
...
contract BinamonCollectionV2 is IERC721Metadata, Pausable {
    ...
    function tokenDetails(uint256 tokenId)
        public
        view
        returns (BinamonTokenV2 memory token, BinamonTypeV2 memory type_)
    {
        require(exists(tokenId), "Nonexistent token");
