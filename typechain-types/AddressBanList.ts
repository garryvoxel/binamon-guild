/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import { FunctionFragment, Result } from "@ethersproject/abi";
import { Listener, Provider } from "@ethersproject/providers";
import { TypedEventFilter, TypedEvent, TypedListener, OnEvent } from "./common";

export interface AddressBanListInterface extends utils.Interface {
  contractName: "AddressBanList";
  functions: {
    "isBanned(address)": FunctionFragment;
  };

  encodeFunctionData(functionFragment: "isBanned", values: [string]): string;

  decodeFunctionResult(functionFragment: "isBanned", data: BytesLike): Result;

  events: {};
}

export interface AddressBanList extends BaseContract {
  contractName: "AddressBanList";
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: AddressBanListInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    isBanned(user: string, overrides?: CallOverrides): Promise<[boolean]>;
  };

  isBanned(user: string, overrides?: CallOverrides): Promise<boolean>;

  callStatic: {
    isBanned(user: string, overrides?: CallOverrides): Promise<boolean>;
  };

  filters: {};

  estimateGas: {
    isBanned(user: string, overrides?: CallOverrides): Promise<BigNumber>;
  };

  populateTransaction: {
    isBanned(
      user: string,
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;
  };
}
